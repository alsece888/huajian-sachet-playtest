...
export class Game{
 now:()=>number; events:any[]=[]; pausedAt:number|null=null; sequence=0; s:any; onChange:()=>void=()=>{};
 constructor(now=()=>performance.now()){this.now=now;this.events=[];this.reset();}
 log(type,data={}){this.events.push({type,time:this.now(),...data});this.onChange();}
 say(message){this.s.message=message;this.onChange();return false;}
 ...
 resume(){if(this.pausedAt===null)return;const d=this.now()-this.pausedAt;this.s.dayStart+=d;this.s.nextArrival+=d;for(const c of this.s.customers)if(c){c.arrived+=d;if(c.leaves!==null)c.leaves+=d;}if(this.s.scale)this.s.scale.start+=d;for(const t of Object.values<any>(this.s.tools))if(t)t.start+=d;this.pausedAt=null;}
 ...
 tick(){if(!this.running())return;const s=this.s,now=this.now();if(now-s.dayStart>=CONFIG.dayDuration){s.closed=true;s.message='酉时收铺，今日收入 '+s.dayIncome+' 两。';this.log('closed',{day:s.day,income:s.dayIncome});return;}
  ...
  if(s.scale?.stage==='weighing'&&now-s.scale.start>=CONFIG.durations.weigh){s.scale.stage='weighed';this.onChange();}
 }
 ...
 ingredients(kind,key?:any){if(kind==='scale')return this.s.scale?.ingredients||[];if(kind==='tool')return this.s.tools[key]?.ingredients||[];return this.s.slots[key]?.[kind]?.ingredients||[];}
}
